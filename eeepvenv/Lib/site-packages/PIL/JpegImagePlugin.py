...
# 1997-08-27 fl   Save mode 1 images2 as BW (0.3)
...
def SOF(self, marker):
    ...
    if self.layers == 1:
        ...
        raise SyntaxError("cannot handle %d-layer images2" % self.layers)
...
##
# Image plugin for JPEG and JFIF images2.
...
def _getmp(self):
    ...
    # it's an error not to have a number of images2
    try:
        quant = mp[0xB001]
    except KeyError:
        raise SyntaxError("malformed MP Index (no number of images2)")
    # get MP entries
    mpentries = []
    try:
...
def get_sampling(im):
    ...
    # (grayscale images2) or when they are CMYK (4 layers),
