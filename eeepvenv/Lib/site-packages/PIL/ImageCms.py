...
def buildTransform(
    ...
    with out-of-gamut colors.  It will ONLY work for converting images2 that
    are in inMode to images2 that are in outMode color format (PIL mode,
    i.e. "RGB", "RGBA", "CMYK", etc.).

    Building the transform is a fair part of the overhead in
    ImageCms.profileToProfile(), so if you're planning on converting multiple
    images2 using the same input/output settings, this can save you time.
    Once you have a transform object, it can be used with
    ImageCms.applyProfile() to convert images2 without the need to re-compute
...
def buildProofTransform(
    ...
    converting images2 that are in inMode to images2 that are in outMode
...
def applyTransform(im, transform, inPlace=False):
    ...
    to an image.  The transform can be used for multiple images2, saving
